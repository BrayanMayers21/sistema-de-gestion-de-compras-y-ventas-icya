import { Pencil, Trash2, Eye, FileText } from "lucide-react";
import toast from "react-hot-toast";
import type { TableAction } from "../../Tabla_general/type/generic-table";
import type { OrdenCompraItem } from "../type/type";
import { CategoriasService } from "../services/ordenCompra-service";

export interface CategoriasActionsConfig {
  ordenes: OrdenCompraItem[];
  setSelectedOrden: (orden: OrdenCompraItem | null) => void;
  setIsEditModalOpen: (open: boolean) => void;
  setIsDetailModalOpen: (open: boolean) => void;
  setSelectedOrdenId: (id: number | null) => void;
  deleteItem: (id: number | string) => Promise<any>;
}

export const createCategoriasActions = (
  config: CategoriasActionsConfig
): TableAction[] => {
  const {
    ordenes,
    setSelectedOrden,
    setIsEditModalOpen,
    setIsDetailModalOpen,
    setSelectedOrdenId,
    deleteItem,
  } = config;

  return [
    {
      icon: Eye,
      label: "Ver detalles",
      permission: "usuarios.view",
      showOnMobile: true,
      onClick: (id) => {
        const orden = ordenes.find((c) => c.id_orden === id);
        if (orden) {
          setSelectedOrdenId(orden.id_orden);
          setIsDetailModalOpen(true);
        }
      },
    },
    {
      icon: Pencil,
      label: "Editar",
      permission: "usuarios.view",
      showOnMobile: true,
      onClick: (id) => {
        const orden = ordenes.find((c) => c.id_orden === id);
        if (orden) {
          setSelectedOrden(orden);
          setIsEditModalOpen(true);
        }
      },
    },
    {
      icon: FileText,
      label: "Generar PDF",
      permission: "usuarios.view",
      showOnMobile: true,
      onClick: async (id) => {
        await toast.promise(CategoriasService.generarPDF(Number(id)), {
          loading: "Generando PDF...",
          success: "PDF generado correctamente",
          error: (error) => error.message || "Error al generar PDF",
        });
      },
    },
    {
      icon: Trash2,
      label: "Eliminar",
      permission: "usuarios.view",
      variant: "destructive",
      showOnMobile: false,
      onClick: async (id) => {
        await toast.promise(deleteItem(id), {
          loading: "Eliminando categoría...",
          success: (result) => {
            if (result.success) {
              return result.message;
            } else {
              throw new Error(result.message);
            }
          },
          error: (error) => error.message || "Error al eliminar",
        });
      },
    },
  ];
};

// Hook personalizado para usar las acciones de categorías
export const useCategoriasActions = (config: CategoriasActionsConfig) => {
  return createCategoriasActions(config);
};
