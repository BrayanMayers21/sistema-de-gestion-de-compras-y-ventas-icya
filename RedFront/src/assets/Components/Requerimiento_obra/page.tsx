...
export default function RequerimientoPage() {
  ...
  const [selectedRequerimiento, setSelectedRequerimiento] =
    useState<RequerimientoObra | null>(null);
  ...
  const {
    ...
    refetch,
    clearFilters,
  } = useGenericList<RequerimientoObra>({
    endpoint: "/api/requerimientos-obra",
  ...
  const handleCreateRequerimiento = async (
    ...
    try {
      ...
      const result = await RequerimientosService.crearRequerimientoObra(data);
  ...
  const handleUpdateRequerimiento = async (
    ...
    try {
      ...
      await RequerimientosService.actualizarRequerimientoObra(
        selectedRequerimiento.id_requerimiento_obra,
        {
          ...data,
          id_requerimiento_obra: selectedRequerimiento.id_requerimiento_obra,
        }
      );
  ...
  return (
    <div className="container mx-auto py-0 space-y-4 p-3 sm:p-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:justify-between lg:items-center">
        <div className="space-y-1">
          ...
          <p className="text-sm sm:text-base text-muted-foreground">
            Administra los requerimientos de materiales para obras.
          </p>
        </div>
      ...
      {/* Tabla genérica */}
      <GenericDataTable<RequerimientoObra>
        columns={columns}
        data={requerimientos}
        actions={actions}
        loading={loading}
        ...
        onRefetch={refetch}
        idField="id_requerimiento_obra"
        pageTitle="Requerimientos de Obra"
      />
